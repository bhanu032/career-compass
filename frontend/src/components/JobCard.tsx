    ...
    <article className="card animate-fade-up flex flex-col p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <Link
            to={`/jobs/${job.id}`}
            className="line-clamp-2 text-base font-semibold text-slate-900 hover:text-brand-600 dark:text-white dark:hover:text-brand-400"
          >
            {job.title}
          </Link>
        ...
        {isAuthenticated && (
          <button
            ...
            className={classNames(
              ...
              bookmarked
                ? "border-brand-200 bg-brand-50 text-brand-600 dark:border-brand-800 dark:bg-brand-900/40 dark:text-brand-300"
      ...
      {!compact && (
        <dl className="mt-4 grid grid-cols-1 gap-2.5 text-sm sm:grid-cols-2">
          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
            ...
            <span className="truncate">{job.qualification ?? "Any graduate"}</span>
          </div>
          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
          ...
          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
            ...
            <span className="truncate">{job.state ?? "All India"}</span>
          </div>
          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {job.vacancies ? <span className="badge">{job.vacancies.toLocaleString("en-IN")} posts</span> : null}
      ...
      <Link to={`/jobs/${job.id}`} className="btn-primary mt-5 w-full">
        View details
      </Link>
    </article>
  );
