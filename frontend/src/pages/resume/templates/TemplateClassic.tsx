      ...
      skills.map((s) => (
        <div key={s.id} style={{ marginBottom: 8 }}>
          ...
          >
            ...
            {showLevels && (
              <span style={{ color: "#64748b", fontSize: 10.5 }}>{s.level}</span>
            )}
          </div>
          <div style={{ height: 4, background: "#dde3ef", borderRadius: 99 }}>
            <div
              style={{
                width: `${levelBar[s.level]}%`,
                height: "100%",
                background: accent,
                borderRadius: 99,
              }}
    ...
    <div className="resume-template" style={wrapStyle}>
      ...
      <div
        style={{
          background: accent,
          color: "#fff",
          padding: `${v}px ${h}px ${Math.round(v * 0.75)}px`,
          flexShrink: 0,
        }}
      >
        ...
        {p.jobTitle && (
          <p style={{ margin: "4px 0 0", fontSize: 13, opacity: 0.85, fontStyle: "italic" }}>
            {p.jobTitle}
          </p>
        )}
        <div
        ...
        <div style={{ flex: 1, padding: `${v}px ${h}px ${v}px ${Math.round(h * 0.75)}px` }}>
          {p.summary && (
            <Section title="Profile Summary" accent={accent} gap={gap}>
              <p style={{ margin: 0, fontSize: 12, color: "#334155", lineHeight: 1.75 }}>
                {p.summary}
              </p>
            </Section>
          )}
              ...
              {projects.map((pr) => (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    ...
                    {pr.technologies && (
                      <span style={{ fontSize: 11, color: accent, fontStyle: "italic" }}>
                        {pr.technologies}
                      </span>
                    )}
                  </div>
