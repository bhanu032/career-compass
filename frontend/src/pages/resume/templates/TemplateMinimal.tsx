...
export function TemplateMinimal({ data, customization, printMode }: Props): JSX.Element {
  ...
  const sectionMap = {
    summary: p.summary ? (
      <Section key="summary" title="Summary" accent={accent} gap={gap}>
    ...
    experience: experience.length > 0 ? (
      <Section key="experience" title="Experience" accent={accent} gap={gap}>
        {experience.map((e) => (
          <div key={e.id} className="resume-item resume-avoid-break" style={{ marginBottom: 13 }}>
            ...
            {e.description && (
              <p style={{ margin: 0, fontSize: 11.5, lineHeight: 1.7, color: "#475569", whiteSpace: "pre-line" }}>
                {e.description}
              </p>
            )}
          </div>
    ...
    education: education.length > 0 ? (
      <Section key="education" title="Education" accent={accent} gap={gap}>
        {education.map((e) => (
          <div key={e.id} className="resume-item resume-avoid-break"
    ...
    skills: skills.length > 0 ? (
      <Section key="skills" title="Skills" accent={accent} gap={gap}>
      ...
      <Section key="projects" title="Projects" accent={accent} gap={gap}>
        {projects.map((pr) => (
          <div key={pr.id} className="resume-item resume-avoid-break" style={{ marginBottom: 10 }}>
            ...
            {pr.description && (
              <p style={{ margin: "3px 0 0", fontSize: 11.5, lineHeight: 1.6, color: "#475569" }}>{pr.description}</p>
            )}
    ...
    certificates: certificates.length > 0 ? (
      <Section key="certificates" title="Certifications" accent={accent} gap={gap}>
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          {certificates.map((c) => (
            <div key={c.id} className="resume-item resume-avoid-break"
  ...
  return (
    <div className="resume-template" style={wrapStyle}>
      ...
      <div style={{ marginBottom: gap, textAlign: "center" }}>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 300, letterSpacing: 4, color: "#0f172a" }}>
          {p.fullName || "YOUR NAME"}
        </h1>
        {p.jobTitle && (
          <p style={{ margin: "6px 0 0", fontSize: 12.5, color: accent, fontWeight: 600, letterSpacing: 1 }}>
            {p.jobTitle.toUpperCase()}
          </p>
        )}
        <div style={{ marginTop: 9, display: "flex", justifyContent: "center", flexWrap: "wrap", gap: "3px 14px", fontSize: 11, color: "#475569" }}>
        ...
        <div style={{ width: 50, height: 2, background: accent, margin: "12px auto 0", borderRadius: 99 }} />
      </div>
